...
class PaytmChecksum {
	...
	static encrypt(input, key) {
		if (!key) {
			console.log("Error: No key provided for encryption.");
			return null;
		}
	...
	static decrypt(encrypted, key) {
		if (!key) {
			console.log("Error: No key provided for decryption.");
			return null;
		}
	...
	static generateSignature(params, key) {
		if (!key) {
			console.log("Error: No key provided for signature generation.");
			return null;
		}
	...
	static verifySignature(params, key, checksum) {
		if (!key) {
			console.log("Error: No key provided for signature verification.");
			return null;
		}
		if (typeof params !== "object" && typeof params !== "string") {
		   	var error = "string or object expected, " + (typeof params) + " given.";
	...
	static getStringByParams(params) {
		...
		Object.keys(params).sort().forEach(function(key) {
			data[key] = (params[key] !== null && params[key] !== undefined && params[key].toString().toLowerCase() !== "null") ? params[key].toString().trim() : "";
		});
